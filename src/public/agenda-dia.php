...
?><a class="agenda-dia-ag agenda-dia-ag--<?= htmlspecialchars($ag['status']) ?>" style="height:<?= number_format($alturaAgendamento-4,2,'.','') ?>px" href="agendamentos.php?data_inicio=<?= $data ?>&amp;data_fim=<?= $data ?>"><strong><?= htmlspecialchars($ag['cliente']) ?></strong><span><?= htmlspecialchars($ag['servico']) ?></span><small><?= $ag['ini']->format('H:i') ?>–<?= $ag['fimdt']->format('H:i') ?></small></a>
...
<div class="agenda-dia-mobile"><?php foreach($profissionais as $p):$pid=(int)$p['id'];?><section class="app-card mb-3"><div class="app-card-header"><strong><?= htmlspecialchars($p['nome_completo']) ?></strong></div><div class="app-card-body"><?php if(empty($ags[$pid])):?><span class="text-muted">Nenhum atendimento.</span><?php else:foreach($ags[$pid] as $a):?><a class="agenda-dia-mobile-item" href="agendamentos.php?data_inicio=<?= $data ?>&amp;data_fim=<?= $data ?>"><strong><?= $a['ini']->format('H:i') ?>–<?= $a['fimdt']->format('H:i') ?></strong><span><?= htmlspecialchars($a['cliente']) ?></span><small><?= htmlspecialchars($a['servico']) ?></small></a><?php endforeach;endif;?></div></section><?php endforeach;?></div>
