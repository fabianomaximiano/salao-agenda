...
-- ----------------------------------------------------------------------------
-- 1.1 IDENTIDADE VISUAL DA EMPRESA
-- ----------------------------------------------------------------------------
--
-- A identidade visual pertence à empresa e pode ser consumida por interfaces
-- internas e, futuramente, pela API pública utilizada pelo site institucional.
-- As cores são tokens de marca opcionais; sua aplicação visual é decisão do
-- sistema consumidor.
-- ----------------------------------------------------------------------------

CREATE TABLE empresa_identidade_visual (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    empresa_id BIGINT UNSIGNED NOT NULL,
    logo_arquivo VARCHAR(500) NULL,
    ...
    CONSTRAINT fk_empresa_identidade_visual_empresa
        FOREIGN KEY (empresa_id) REFERENCES empresas(id)
        ON DELETE CASCADE,

    UNIQUE KEY uq_empresa_identidade_visual_empresa (empresa_id)
) ENGINE=InnoDB;
